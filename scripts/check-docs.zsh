...
#
# Two passes:
#   1. Every relative markdown link/image in the docs resolves to an existing
#      file, and its #anchor (if any) to a real heading.
#   2. Site reachability: every relative .md link in a file on the Quarto
#      render list targets another rendered file, so no link 404s on the
#      published site.
#
...
local f l t
local -a files links
files=( README.md index.md(N) EXAMPLES.md(N) CHANGELOG.md(N) docs/*.md(N) )
for f in $files; do
